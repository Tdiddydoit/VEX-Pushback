...
def Autonomus():
    ...
    bot = Bot_Motors()
    drive_list = bot.Drivetrain()  # returns [m1, m2, m3, m4, left_group, right_group]

    # The first four items are the individual motors
    motors = drive_list[0:4]
...
def User_Control():
    ...
    DriveTrain = Bot_Motors()
    drive_list = DriveTrain.Drivetrain()


    # grab left and right side from dirvetrain class
    left_side = drive_list[4]
    right_side = drive_list[5]
...
if __name__ == "__main__":
    def main():
        ...
        # main will never make it program stop due to while loop in user control timeout can be implemented
        Autonomus()
        User_Control()
        brain.program_stop()
    main()
